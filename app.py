...
def init():
    ...
    if c.execute("SELECT COUNT(*) FROM users").fetchone()[0]==0:
        c.executemany("INSERT INTO users(name,code,password,role) VALUES(?,?,?,?)",[
            ("Quản trị viên","admin","admin123","admin"),("Giáo viên Demo","gv001","123456","teacher"),("Nguyễn Minh Anh","hs001","123456","student")])
...
def inject(): return {"me":current()}

@app.route("/")
def home():
    c=db()
    data={"books":c.execute("SELECT * FROM books ORDER BY id DESC").fetchall(),
    ...
    c.close(); return render_template("home.html",**data)

@app.route("/login",methods=["GET","POST"])
def login():
    if request.method=="POST":
        code=request.form["code"].strip(); pw=request.form["password"]
        c=db(); u=c.execute("SELECT * FROM users WHERE code=? AND password=?",(code,pw)).fetchone(); c.close()
        if u:
            session["user"]=dict(u); return redirect(url_for("home"))
...
@app.route("/books")
def books():
    c=db(); rows=c.execute("SELECT * FROM books ORDER BY id DESC").fetchall(); c.close()
    return render_template("books.html",books=rows)

@app.route("/assignments")
def assignments():
    c=db(); rows=c.execute("SELECT * FROM assignments ORDER BY id DESC").fetchall(); c.close()
...
def assignment(aid):
    ...
    c=db(); a=c.execute("SELECT * FROM assignments WHERE id=?",(aid,)).fetchone()
    if request.method=="POST":
        c.execute("INSERT INTO submissions(assignment_id,user_id,answer) VALUES(?,?,?)",(aid,current()["id"],request.form["answer"]))
        c.commit(); flash("Đã nộp bài."); return redirect(url_for("assignments"))
    c.close(); return render_template("assignment.html",a=a)

@app.route("/videos")
def videos():
    c=db(); rows=c.execute("SELECT * FROM videos ORDER BY id DESC").fetchall(); c.close()
...
def journal():
    ...
    c=db()
    if request.method=="POST":
        c.execute("INSERT INTO journals(user_id,book,content) VALUES(?,?,?)",(current()["id"],request.form["book"],request.form["content"]))
        c.commit(); flash("Đã lưu nhật ký đọc sách.")
    rows=c.execute("SELECT * FROM journals WHERE user_id=? ORDER BY id DESC",(current()["id"],)).fetchall()
    c.close(); return render_template("journal.html",journals=rows)
...
def add_book():
    ...
    c=db(); c.execute("INSERT INTO books(title,author,category,description) VALUES(?,?,?,?)",(request.form["title"],request.form["author"],request.form["category"],request.form["description"])); c.commit(); c.close()
...
def add_assignment():
    ...
    if current()["role"]=="teacher" and cls and cls not in teacher_classes():
        flash("Bạn không được giao bài cho lớp này."); return redirect(url_for("admin"))
    c=db(); c.execute("INSERT INTO assignments(title,description,due,points,class_name) VALUES(?,?,?,?,?)",
...
def add_video():
    if not teacher_required(): return redirect(url_for("login"))
    c=db(); c.execute("INSERT INTO videos(title,description,url,file) VALUES(?,?,?,?)",(request.form["title"],request.form["description"],request.form["url"],"")); c.commit(); c.close()
    return redirect(url_for("admin"))

@app.route("/admin/score/<int:sid>",methods=["POST"])
def score(sid):
    if not teacher_required(): return redirect(url_for("login"))
    c=db(); c.execute("UPDATE submissions SET score=?,status=? WHERE id=?",(request.form["score"],"Đã chấm",sid)); c.commit(); c.close()
    return redirect(url_for("admin"))

@app.route("/leaderboard")
def leaderboard():
    c=db(); rows=c.execute("""SELECT u.name, COALESCE(SUM(s.score),0) score FROM users u LEFT JOIN submissions s ON u.id=s.user_id WHERE u.role='student' GROUP BY u.id ORDER BY score DESC""").fetchall(); c.close()
    return render_template("leaderboard.html",rows=rows)

@app.route("/students")
def students():
    if not teacher_required(): return redirect(url_for("login"))
    c=db(); rows=c.execute("SELECT * FROM users WHERE role='student' ORDER BY class_name,name").fetchall()
    classes=c.execute("SELECT * FROM classes ORDER BY name").fetchall(); c.close()
...
def profile():
    if not current(): return redirect(url_for("login"))
    c=db(); u=c.execute("SELECT * FROM users WHERE id=?",(current()["id"],)).fetchone(); c.close()
    return render_template("profile.html",u=u)

@app.route("/change-password",methods=["POST"])
def change_password():
    if not current(): return redirect(url_for("login"))
    c=db(); c.execute("UPDATE users SET password=? WHERE id=?",(request.form["password"],current()["id"])); c.commit(); c.close()
    flash("Đã đổi mật khẩu."); return redirect(url_for("profile"))

@app.route("/notifications")
def notifications():
    if not current(): return redirect(url_for("login"))
    c=db(); rows=c.execute("SELECT * FROM notifications WHERE user_id=? ORDER BY id DESC",(current()["id"],)).fetchall(); c.execute("UPDATE notifications SET read=1 WHERE user_id=?",(current()["id"],)); c.commit(); c.close(); return render_template("notifications.html",notifications=rows)

@app.route("/admin/notify",methods=["POST"])
def notify():
    if not teacher_required(): return redirect(url_for("login"))
    c=db(); users=c.execute("SELECT id FROM users WHERE role='student'").fetchall()
    for u in users: c.execute("INSERT INTO notifications(user_id,title,message) VALUES(?,?,?)",(u["id"],request.form["title"],request.form["message"]))
    c.commit(); c.close(); flash("Đã gửi thông báo."); return redirect(url_for("admin"))

@app.route("/badges")
def badges():
    if not current(): return redirect(url_for("login"))
    c=db(); rows=c.execute("SELECT b.*, CASE WHEN ub.user_id IS NULL THEN 0 ELSE 1 END earned FROM badges b LEFT JOIN user_badges ub ON b.id=ub.badge_id AND ub.user_id=?",(current()["id"],)).fetchall(); c.close(); return render_template("badges.html",badges=rows)

@app.route("/admin/badge",methods=["POST"])
def add_badge():
    if not teacher_required(): return redirect(url_for("login"))
    c=db(); c.execute("INSERT INTO badges(name,description,icon,points) VALUES(?,?,?,?)",(request.form["name"],request.form["description"],request.form["icon"],request.form["points"])); c.commit(); c.close(); return redirect(url_for("admin"))

@app.route("/upload",methods=["POST"])
def upload():
    if not teacher_required(): return redirect(url_for("login"))
    f=request.files.get("file")
    if not f or not f.filename: flash("Chưa chọn file."); return redirect(url_for("admin"))
    allowed={"pdf","doc","docx","jpg","jpeg","png","mp4","webm"}; ext=f.filename.rsplit(".",1)[-1].lower() if "." in f.filename else ""
    if ext not in allowed: flash("Định dạng chưa được hỗ trợ."); return redirect(url_for("admin"))
    name=secure_filename(f.filename); f.save(os.path.join(UPLOAD,name)); flash("Đã tải file lên."); return redirect(url_for("admin"))
...
def teacher_classes():
    ...
    if current()["role"]=="admin": 
        c=db(); rows=c.execute("SELECT name FROM classes ORDER BY name").fetchall(); c.close()
        return [r["name"] for r in rows]
    c=db(); rows=c.execute("SELECT class_name FROM teacher_classes WHERE teacher_id=?",(current()["id"],)).fetchall(); c.close()
...
def dashboard():
    ...
    if current()["role"]=="student":
        stats={
            ...
            "assignments":c.execute("SELECT COUNT(*) FROM submissions WHERE user_id=?",(current()["id"],)).fetchone()[0],
            "journals":c.execute("SELECT COUNT(*) FROM journals WHERE user_id=?",(current()["id"],)).fetchone()[0],
            "score":c.execute("SELECT COALESCE(SUM(score),0) FROM submissions WHERE user_id=?",(current()["id"],)).fetchone()[0]
        }
    else:
        stats={
...
def teacher_class():
    if current()["role"]!="admin": return redirect(url_for("login"))
    c=db()
    try:
        c.execute("INSERT INTO teacher_classes(teacher_id,class_name) VALUES(?,?)",(request.form["teacher_id"],request.form["class_name"]))
        c.commit()
...
def reports():
    if not teacher_required(): return redirect(url_for("login"))
    c=db()
    classes=c.execute("""SELECT class_name, COUNT(*) students FROM users WHERE role='student' GROUP BY class_name ORDER BY class_name""").fetchall()
    activity=c.execute("""SELECT a.class_name, COUNT(s.id) submissions, COALESCE(SUM(s.score),0) score
                         FROM assignments a LEFT JOIN submissions s ON a.id=s.assignment_id
                         GROUP BY a.class_name ORDER BY a.class_name""").fetchall()
    c.close()
